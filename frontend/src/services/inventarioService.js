...
export function listarRefrigerantes() {
  return Promise.resolve(dataSource === 'mock' ? mockStore.refrigerants() : apiRequest(`${apiEndpoints.materials}?categoria=REFRIGERANTE`))
}
...
export function registrarConsumoRefrigerante({ ordenId, refrigeranteId, cantidad }) {
  ...
  return apiRequest(`${endpointWithId(apiEndpoints.workOrders, ordenId)}/materiales`, { method: 'POST', body: JSON.stringify({ materialId: refrigeranteId, cantidad }) })
}
